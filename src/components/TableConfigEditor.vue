...
import type { TableConfig, SearchOption } from '@/types';
...
const currentNestedConfig = ref<TableConfig | null>(null);

const openNestedConfig = (formConfig: any) => {
...
const isBorderedCell = ref(typeof props.config.bordered === 'object' ? props.config.bordered.cell : (props.config.bordered || false));
...
const addOption = (formConfig: any) => {
  ...
  <div class="table-config-editor">
    <a-tabs default-active-key="1">
      ...
      <a-tab-pane key="2" title="列配置">
        ...
        <a-collapse :default-active-key="['0']" accordion>
          <a-collapse-item
            ...
            :key="String(index)"
            :header="`${col.title || '未命名'} (${col.dataIndex})`"
          >
            <template #extra>
              <a-button status="danger" size="mini" @click.stop="removeColumn(index)"
                >删除</a-button
              >
            </template>
            <a-form :model="col" layout="vertical">
              ...
              >
                  ...
                  <a-col :span="8">
                    <a-form-item label="类型 (Type)">
                      <a-select v-model="col.form.type">
                        <a-option v-for="t in formTypes" :key="t" :value="t">{{
                          t
                        }}</a-option>
                      </a-select>
                    </a-form-item>
                ...
                <div
                  v-if="
                    col.form.type &&
                    ['select', 'radio', 'checkbox'].includes(col.form.type)
                  "
                  style="margin-top: 16px"
                >
                  <a-divider orientation="left" style="margin: 10px 0"
                    >选项配置 (Options)</a-divider
                  >
                  <div
                    v-for="(opt, oIdx) in col.form.options"
                    :key="oIdx"
                    style="display: flex; gap: 8px; margin-bottom: 8px"
                  >
                    <a-input v-model="opt.label" placeholder="Label" />
                    <a-input v-model="opt.value" placeholder="Value" />
                    <a-button status="danger" @click="(col.form!.options as SearchOption[]).splice(oIdx, 1)">
                      <template #icon>x</template>
                    </a-button>
                  </div>
                  <a-button type="dashed" long @click="addOption(col.form)"
                    >+ 添加选项</a-button
                  >
                </div>
                ...
                <div v-if="col.form.type === 'table'" style="margin-top: 10px">
                  <a-alert type="info" style="margin-bottom: 10px"
                    >这是一个嵌套表格字段。</a-alert
                  >
                  <a-button type="primary" @click="openNestedConfig(col.form)"
                    >编辑子表格配置 (Table Config)</a-button
                  >
                </div>
              </div>
              <div v-else>
                <a-button
                  size="mini"
                  @click="col.form = { type: 'input', creatable: true, editable: true }"
                  >启用表单配置</a-button
                >
              </div>
            </a-form>
      ...
      <a-tab-pane key="3" title="搜索配置">
        ...
        <a-collapse :default-active-key="['0']" accordion>
          <a-collapse-item
            v-for="(field, index) in config.searchFields"
            :key="String(index)"
            :header="`${field.title || '未命名'} (${field.dataIndex})`"
          >
            <template #extra>
              <a-button status="danger" size="mini" @click.stop="removeSearchField(index)"
                >删除</a-button
              >
            </template>
            <a-form :model="field" layout="vertical">
              ...
              <!-- 选项配置 -->
              <div
                v-if="field.type && ['select', 'radio', 'checkbox'].includes(field.type)"
                style="margin-top: 16px"
              >
                <a-divider orientation="left" style="margin: 10px 0"
                  >选项配置 (Options)</a-divider
                >
                <div
                  v-for="(opt, oIdx) in (field.options as SearchOption[])"
                  :key="oIdx"
                  style="display: flex; gap: 8px; margin-bottom: 8px"
                >
                  <a-input v-model="opt.label" placeholder="Label" />
                  ...
                  <a-button
                    status="danger"
                    @click="(field.options as SearchOption[]).splice(oIdx, 1)"
                  >
                    <template #icon>x</template>
                  </a-button>
                </div>
                <a-button type="dashed" long @click="addOption(field)"
                  >+ 添加选项</a-button
                >
              </div>
        ...
        <a-form :model="config" layout="vertical">
          <a-form-item label="列表接口 (apiUrl / pageApiUrl)">
            ...
            <a-input
              ...
              placeholder="API URL (Backend pagination)"
